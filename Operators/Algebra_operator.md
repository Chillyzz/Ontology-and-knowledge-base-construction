...
1. GreaterThan: Greaterthan({a: Number}, {b: Number}) -> Boolean  # Greaterthan({3: Number}, {5: Number}) -> False.
    备注: 判断 a > b 是否成立, 成立返回 True 反正 False, 下同

2. LessThan: Lessthan({a: Number}, {b: Number}) -> Boolean # Lessthan({3: Number}, {5: Number}) -> True

3. GreaterOrEqualThan: GreaterOrEqualThan({a: Number}, {b: Number}) -> Boolean # GreaterOrEqualThan({5: Number}, {5: Number}) -> True

4. LessOrEqualThan: LessOrEqualThan({a: Number}, {b: Number}) -> Boolean # LessOrEqualThan({4: Number}, {5: Number}) -> True
...
2. Solve_equation: Solve_equation(Equation(P), Variable(x)) -> Set # 输入一个等式和对应变量, 返回解集

3. Solve_inequation: Solve_inequation(Inequation(P), Variable(x)) -> Set # 输入一个不等式和对应变量, 返回解集

4. Get_Value_Expression: Get_Value_Expression(Expression(P), Variable(x)) -> Number # 输入一个表达式和变量, 返回它的值
...
(暂定)
5. Get_Function_Continuity: Get_Function_Continuity(f: Function) -> Boolean  
   备注: 输入一个函数, 判断该函数在其定义域内是否连续。返回布尔值，`True`表示函数在定义域内连续，`False`表示不连续。  
...
10. Get_Function_Parity: Get_Function_Parity(f: Function) -> String  
    备注: 输入一个函数, 判断该函数是奇函数、偶函数还是既不是奇函数也不是偶函数。返回字符串 "odd"、"even" 或 "none"。  

11. Get_Function_Symmetry: Get_Function_Symmetry(f: Function) -> String  
    备注: 输入一个函数, 判断该函数是否具有对称性。返回字符串 "symmetrical" 或 "asymmetrical"。  

12. Get_Function_Zeroes: Get_Function_Zeroes(f: Function) -> Set  
